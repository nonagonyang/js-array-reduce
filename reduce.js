...
which accepts an array of objects and a key and returns a new array with the value of each object at the key.
...
and returns an object with the keys as the vowel and the values as the number of times the vowel appears
in the string. 
...
function vowelCount(str) {
    ...
    let vowels="aeiou";
    let vArr= arr.filter(function(char){
        return vowels.indexOf(char)!==-1})

    return vArr.reduce(function(acc,nextValue){
        if(acc[nextValue]){
            acc[nextValue]+=1;
        }
        else {
...
function partition(arr, callback) {
    return arr.reduce(function(accu,nextValue){
        if(callback(nextValue)){
            accu[0].push(nextValue);
        }
        else {
            accu[1].push(nextValue)
        }
        return accu;
